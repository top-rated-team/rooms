...
const TOKEN_WARNING =
  "This room's address does not go out over WhatsApp, Slack or ClickUp. A link in a group chat is the account.";
...
const KINDS: { kind: BridgeKind; name: string; targetHint: string }[] = [
  ...
  { kind: "chatwoot", name: "ChatWoot", targetHint: "One conversation id, not the whole inbox" },
...
export function BridgePanel({ token, className }: BridgePanelProps) {
              ...
              <li key={row.kind} className="border-t border-border py-3" data-testid={`bridge-${row.kind}`}>
                ...
                {connected ? (
                  ...
                  >
                    ...
                    {row.kind !== "whatsapp" && row.kind !== "slack" ? (
                      <label className="mt-2 block">
